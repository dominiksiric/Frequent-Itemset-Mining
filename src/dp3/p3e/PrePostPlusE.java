...
public class PrePostPlusE extends FISMAlgorithm {
	...
	public int countSupportFor2Itemset(String item1, String item2) throws IOException {
		// Call countSupportCountForItems to get the support count for each item
		countSupportCountForItems();

		// Initialize the support count for the 2-itemset to 0
		int support = 0;
		...
		while ((line = input.readLine()) != null) {
			...
			// If both items are found in the transaction, increment the support count
		...
		// Return the support count for the 2-itemset
		return support;
	}
	...
	public long prepareToConstructPPCTree(List<String> globalFrequentItems) {
		...
		// Initialize itemsMaptoFrequencies here
		this.itemsMaptoFrequencies = new HashMap<>();

		int size = globalFrequentItems.size();
		this.globalIndex_item = globalFrequentItems;
		this.item_globalIndex = new HashMap<>(size);
		this.itemCodesMaptoFrequencies = new HashMap<>(size);
		...
		// itemsMaptoFrequencies initialization is already done
    ...
    public long constructPPCTree() throws IOException {
		...
		// Ensure that globalIndex_item is initialized before accessing its size
		if (this.globalIndex_item == null) {
			// Initialize globalIndex_item if it is null
			this.globalIndex_item = new ArrayList<>();
		}
     ...
     */
    public long create_nodelist_for_items(){
    	long start = System.currentTimeMillis();
     ...
     * @throws InterruptedException
     */
	public long generate_frequent_2itemsets() throws InterruptedException{
    	long start = System.currentTimeMillis();
